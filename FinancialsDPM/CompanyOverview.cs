...
namespace FinancialsDPM
{
    ...
    public static class GetCompanyOverviewData
    {
        ...
        public static async Task InitData()
        {
            ...
            string[] stocklist = {"AMZN", "GOOGL", "AAPL", "FB", "NFLX"};
            foreach (var stock in stocklist)
            {
                var rawData = await GetCompanyOverviewData.CompanyOverviewRequest(stock, null, new CancellationToken());
                var processedData = GetCompanyOverviewData.ProcessCompanyOverviewData(rawData)[0];
                Console.WriteLine(processedData.Address);
                FinancialsUpdateTables.UpdateCompanyOverviewTableTable(processedData);
            }
